...
export const ConfigSchema = z
  .object({
    ...
    MODERATION_REPORT_RETENTION_DAYS: z.coerce.number().int().min(1).max(365).default(90),
    MODERATION_REPORTS_PER_SESSION_PER_DAY: z.coerce.number().int().min(1).max(20).default(3),
    MODERATION_REPORTS_PER_IP_PER_DAY: z.coerce.number().int().min(1).max(50).default(5),
