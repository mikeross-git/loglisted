...
const scoreLabels: [keyof AnalysisResult["categoryScores"], string][] = [
  ["premise", "Premise"],
  ["story", "Story"],
  ["structure", "Structure"],
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (authorization.cachedResultAvailable) {
        ...
        setResult(await api.getResult(authorization.resultId, authorization.resultAccessToken));
      } else {
        ...
        setPhase("uploading");
        setPhase("processing");
        setResult(await api.analyze(file, authorization.uploadToken));
      }
      setPhase("completed");
    } catch (caught) {
      resetTurnstile();
    ...
    >
      <header className="loglisted-uploader__header">
        ...
        <p className="loglisted-uploader__privacy-notice">
          Your uploaded PDF is processed temporarily and is not retained after analysis. Scores and
          limited project metadata may be stored according to the{" "}
          <a href={privacyPolicyUrl}>Privacy Policy</a>.
        </p>
      </header>

      <div className="loglisted-uploader__form">
        <section className="loglisted-uploader__section" aria-labelledby="project-details-heading">
          ...
          <div className="loglisted-uploader__field-grid">
            ...
            <div className="loglisted-uploader__field">
              ...
              <input
                ...
                aria-describedby={
                  project.email.length > 0 && !emailIsValid ? "loglisted-email-error" : undefined
                }
                maxLength={254}
                required
              />
            ...
            <div className="loglisted-uploader__field">
              ...
              <select
                ...
                onChange={(event) => updateProject("format", event.target.value as ProjectFormat)}
              >
            ...
            <div className="loglisted-uploader__field loglisted-uploader__field--wide">
              <label className="loglisted-uploader__label" htmlFor="loglisted-logline">
                Logline <span className="loglisted-uploader__optional">(optional)</span>
              </label>
              <textarea
          ...
          <div className="loglisted-uploader__upload-zone">
            ...
            <div id="loglisted-file-status" aria-live="polite">
              ...
              {inspection?.readableTextWarning && (
                <p className="loglisted-uploader__file-status">
                  Readable text could not be confirmed in the browser. The server will verify the
                  PDF before analysis.
                </p>
              )}
            </div>
        ...
        <section className="loglisted-uploader__section" aria-labelledby="confirmations-heading">
        ...
        {error && (
          <p className="loglisted-uploader__error-message" id="loglisted-submit-error" role="alert">
            {error}
          </p>
        )}
        <button
      ...
      {result && (
        <section
          className="loglisted-uploader__result-card"
          aria-labelledby="screenplay-score-heading"
        >
          <h3 className="loglisted-uploader__result-title" id="screenplay-score-heading">
            Screenplay score
          </h3>
          {result.evaluationMode === "mock" && (
            <p className="loglisted-uploader__mock-result" role="status">
              Development fixture result — this is not a genuine screenplay evaluation.
            </p>
          )}
          <div className="loglisted-uploader__score-grid">
            {scoreLabels.map(([key, label]) => (
              <div className="loglisted-uploader__score-row" key={key}>
                <span>{label}</span>
                <strong className="loglisted-uploader__score-value">
                  {result.categoryScores[key].toFixed(1)}
                </strong>
              </div>
            ))}
            <div className="loglisted-uploader__score-row loglisted-uploader__overall-score">
